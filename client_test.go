...
func TestSetTLSFingerprintSpec(t *testing.T) {
	...
	if _, err := c.R().Get("https://tls.browserleaks.com/json"); err != nil {
		t.Errorf("TestSetTLSFingerprintSpec failed: %v", err)
	}
	if _, err := c.R().Get("https://tools.scrapfly.io/api/fp/ja3"); err != nil {
		t.Errorf("TestSetTLSFingerprintSpec failed: %v", err)
	}
}
